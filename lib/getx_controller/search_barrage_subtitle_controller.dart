...
import 'package:jinvideo/model/barrage_subtitle_model.dart';
...
class SearchBarrageSubtitleController extends GetxController {
  ...
  void bindBarrage(String barragePath, FileModel fileModel) {
    ...
    // 将弹幕文件路径写入存储
    DanmakuMMKVCache.getInstance().setString(CacheConst.cachePrev + fileModel.path, barragePath);
  ...
  void unbindBarrage(FileModel fileModel) {
    ...
    // 将弹幕文件路径写入存储
    DanmakuMMKVCache.getInstance().removeKey(CacheConst.cachePrev + fileModel.path);
    Get.find<VideoFileController>().videoFileList.refresh();
    MediaData.playFileListMap[fileModel.directory] = Get.find<VideoFileController>().videoFileList;
    updateVideoFileModel(fileModel);
  }
