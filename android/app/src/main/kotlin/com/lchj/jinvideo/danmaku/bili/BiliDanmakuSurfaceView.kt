...
class BiliDanmakuSurfaceView(
    ...
    companion object {
        // 弹幕速度列表，数字越小越快，越大的越慢
//        private val danmakuSpeedList: List<Float> = listOf(1.5f, 1.25f, 1.0f, 0.75f, 0.5f)
        private val danmakuSpeedList: List<Float> = listOf(2.0f, 1.75f, 1.5f, 1.25f, 1.0f)
...
//                    Log.d("DFM", "danmakuShown(): text=" + danmaku.text);
    ...
    override fun setDanmakuAlphaRatio(danmakuAlphaRatio: Int) {
        if (mDanmakuView.isPrepared) {
            try {
                ...
                Log.e(LogTagUtils.BILI_DANMAKU_LOG_TAG, "setDanmakuAlphaRatio error: $e")
            }
        }
    }
    ...
    override fun setDanmakuScaleTextSize(danmakuFontSizeRatio: Int) {
        Log.e(LogTagUtils.BILI_DANMAKU_LOG_TAG, "setDanmakuScaleTextSize fontSize: ${danmakuFontSizeRatio}, mDanmakuView.isPrepared:${mDanmakuView.isPrepared}")
        if (mDanmakuView.isPrepared) {
            try {
                var fontSizeRatio = (danmakuFontSizeRatio / 100.0f).toFloat()
                Log.e(LogTagUtils.BILI_DANMAKU_LOG_TAG, "setDanmakuScaleTextSize fontSizeRatio: $fontSizeRatio")
    ...
    override fun setRollDanmakuVisibility(visible: Boolean) {
        Log.d(LogTagUtils.BILI_DANMAKU_LOG_TAG, "setL2RDanmakuVisibility visible: $visible, isPrepared: ${mDanmakuView.isPrepared}")
        if (mDanmakuView.isPrepared) {
            Log.d(LogTagUtils.BILI_DANMAKU_LOG_TAG, "setL2RDanmakuVisibility entry")
            try {
